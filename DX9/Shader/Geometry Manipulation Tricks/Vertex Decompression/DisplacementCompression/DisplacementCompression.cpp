...
void CMyD3DApplication::CreateMandelbrot(unsigned int dwSize) //만델브로트 프렉탈생성.. 
{
	...
	for (DWORD z = 0; z < dwSize; z++) {
		...
		for (DWORD x = 0; x < dwSize; x++) {
			...
			unsigned loop;
			for (loop = 0; loop < 1000; loop++) { // 더 자세한 내용을 위해 반복 횟수가 증가했습니다.
				float zre2 = zre * zre;
				float zim2 = zim * zim;
				if (zre2 + zim2 > 4.0f) break;	// 탈출 조건
				zim = 2 * zre * zim + cim;
				zre = zre2 - zim2 + cre;
			}
			val = loop / 1000.0f; // 값을 [0, 1]로 정규화합니다.
